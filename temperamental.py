...
def keyboard_mouse_events(event):
    ...
    if event.ui_element == GUI.SMT_BUTTON:
        if subprocess.getoutput('cat /sys/devices/system/cpu/smt/control') == 'on':
            ...
            SMT = 'on'
        subprocess.run('/usr/bin/echo ' + SMT +
                       ' > /sys/devices/system/cpu/smt/control', shell=True)
    ...
    if event.ui_element == GUI.BOOST_BUTTON:
        if subprocess.getoutput('cat /sys/devices/system/cpu/cpufreq/boost') == '1':
            ...
            BOOST = '1'
        subprocess.run('/usr/bin/echo ' + BOOST +
                       ' > /sys/devices/system/cpu/cpufreq/boost', shell=True)
    ...
    if event.ui_element == GUI.POSITIVE_INCREMENT_BUTTON and GUI.currentTDP != int(MAX_TDP) // 1000:
        GUI.currentTDP = GUI.currentTDP + 1
        PARAMS = "--stapm-limit=" + str(GUI.currentTDP * 1000) + " " + "--fast-limit=" + str(
            GUI.currentTDP * 1000) + " " + " --slow-limit=" + str(GUI.currentTDP * 1000) + " " + " --tctl-temp=90"
    ...
    if event.ui_element == GUI.APPLY_CHANGES:
        ...
        subprocess.run(RYZENADJ + ' ' + "--stapm-limit=" + TARGET_TDP + " " + "--fast-limit=" +
                       TARGET_TDP + " " + "--slow-limit=" + TARGET_TDP + " " + "--tctl-temp=90", shell=True)
